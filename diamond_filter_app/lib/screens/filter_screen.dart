  ...
  void _addRange() {
    ...
    if (min != null && max != null && min < max) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Enter valid min and max values")),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            const Text("Shapes", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            ...
            Row(
              children: [
                _buildTextField("Min", "0.60",minController),
                const SizedBox(width: 8),
                _buildTextField("Max", "3.80",maxController),
                const SizedBox(width: 8),
                _buildAddButton(),
              ],
            ),
                ...
                Expanded(
                  child: TextFormField(
                    ...
                    controller: TextEditingController(
                      text: _caratRanges.isNotEmpty ? _caratRanges.last : '',
                    ),
                    decoration: InputDecoration(
                      ...
                      suffixIcon: _caratRanges.isNotEmpty
                          ? IconButton(
            ...
            Row(
              children: labOptions.entries.map((entry) {
                return Row(
                  children: [
                    Checkbox(
                      value: entry.value,
                      onChanged: (val) {
                        if (val == true) {
                          // Only allow one lab to be selected
                          setState(() {
                            labOptions.updateAll((key, value) => false);
                            labOptions[entry.key] = true;
                          });
                        } else {
                          // Allow deselect
                          setState(() {
                            labOptions[entry.key] = false;
                          });
                        }
                      },
                    ),
            ...
            children: [
              ...
              Wrap(
                ...
                children: colorOptions
                    .map((color) => _buildSingleSelectChip(color, selectedColor, (val) => selectedColor = val))
                    .toList(),
              ),
              const SizedBox(height: 16),
              const Text("Clarity", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: clarityOptions
                    .map((clarity) => _buildSingleSelectChip(clarity, selectedClarity, (val) => selectedClarity = val))
                    .toList(),
              ),
            ],
          ),
